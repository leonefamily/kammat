# -*- coding: utf-8 -*-
"""
Created on Tue Jul 23 18:24:26 2024
...
APP_NAME = 'pt_counts'
HIDDEN_BEFORE_LOAD = ['-AGGTEXT-', '-AGG-', '-LINKIDTEXT-', '-FILLER-',
                      '-LINKID-', '-LINESTEXT-', '-LINES-', '-FIND-']
...
def main(
    ...
    layout = [
        ...
        [sg.Text('Aggregation', size=10, key='-AGGTEXT-', visible=False),
         sg.Slider(range=(300, 7200), default_value=1200, visible=False,
    ...
    while True:
        ...
        if event == '-LOAD-':
            ...
            # if link_id == values['-LINKID-'] and line_ids == window['-LINES-']:
            #     window['-INFO-'].update(
            #         'Trying to plot the same link and lines', text_color='black'
            #         )
            #     continue
            save_settings(window, APP_NAME)
            try:
                ...
                if values['-LINKID-']:
                    ...
                    update_visibility(window, HIDDEN_BEFORE_PLOT_LINE, False)
                    # window['-LINETAB-'].hide_row()
                    # window['-SAVELINETAB-'].hide_row()
                else:
                    ...
                    update_visibility(window, HIDDEN_BEFORE_PLOT_LINK, False)
                    # window['-LINKTAB-'].hide_row()
                    # window['-SAVELINKTAB-'].hide_row()
